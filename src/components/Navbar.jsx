...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Slide-out Navigation Menu - Fixed with proper z-index */}
      <div
      ...
      >
        ...
        {/* Navigation Panel - Starts from top:0 */}
        <div
          className={`absolute top-0 left-0 h-full w-80 lg:w-96 bg-white shadow-2xl transform transition-transform duration-500 ease-in-out overflow-hidden ${
            isOpen ? "translate-x-0" : "translate-x-full"
          }`}
        >
          {/* Header - Fixed at top */}
          <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gradient-to-r from-[#6DAA44] to-emerald-600 sticky top-0 z-10">
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-white font-bold text-lg lg:text-xl">Navigation</span>
            </div>
            <button
              onClick={closeAllMenus}
          ...
          <div className="h-[calc(100%-120px)] overflow-y-auto pt-4">
                  ...
                  {item.submenu ? (
                    <div className="space-y-1">
                      <button
                        ...
                        className={`flex items-center justify-between w-full px-4 py-3 lg:py-4 rounded-lg transition-all duration-200 font-medium ${
                        ...
                        <div className="pl-4 space-y-1 mt-1">
                          {item.submenu.map((subItem, subIndex) => (
                            <Link
                              ...
                              to={subItem.path}
                              onClick={closeAllMenus}
                              className={`block px-4 py-2 lg:py-3 rounded-lg transition-all duration-200 text-sm lg:text-base ${
                    ...
                    <Link
                      to={item.path}
                      onClick={closeAllMenus}
                      className={`block px-4 py-3 lg:py-4 rounded-lg transition-all duration-200 font-medium text-base lg:text-lg ${
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 bg-gray-50 border-t border-gray-200">
            <div className="text-center">
              <Link
                to="/contact"
                onClick={closeAllMenus}
                className="inline-flex items-center justify-center gap-2 bg-[#6DAA44] text-white px-6 py-3 lg:py-4 rounded-lg font-semibold hover:bg-emerald-600 transform hover:scale-105 transition-all duration-200 w-full text-base lg:text-lg"
              >
                Get In Touch
              </Link>
            </div>
